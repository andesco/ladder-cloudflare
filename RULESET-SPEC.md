...
2. Match first rule by host (exact or suffix), optional `paths`, and optional `pathExclusions`.
...
5. If response is HTML, apply transforms in this order:
   ...
   4. `csCode`
   5. `ampUnhide`
   6. proxy URL rewrite (`src`, `href`, `action`, CSS `url()`)
   7. `injections`
   8. `clearStorage`
...
- Ruleset mode key: `bpc:ruleset_bpc:mode` (`bpc+ladder-v2`).
